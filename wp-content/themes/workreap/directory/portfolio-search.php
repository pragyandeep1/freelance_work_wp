					...
					<form method="GET" class="wt-formtheme wt-formsearch portfolio-submit">
							...
							<ul class="wt-navarticletab">
								<li>
									<span class="wt-radio">
										...
										<label for="cat-0"><?php echo esc_html('All', 'workreap'); ?></label>
									</span>
								</li>
						...
						<fieldset>
							<div class="form-group">
								<input type="text" name="tag" value="<?php echo esc_attr($tag); ?>" class="form-control" placeholder="<?php echo esc_attr('Search tag here', 'workreap') ?>">
						...
						<ul class="wt-search-items">
							...
							while ($portfolio_data->have_posts()) : $portfolio_data->the_post();
								...
								if( ( count( $gallery_imgs ) > 0 ) || ( count( $videos ) > 0 ) ) {
									...
									$class	= 'wt-portfolio-listing-'.intval($flag).' owl-carousel';
								} else {
									$class	= '';
								}
								?>
...
jQuery(document).on('ready', function() {
	jQuery('.wt-portfolio-listing-".esc_js($flag)."').owlCarousel({
		items: 1,
		loop:true,
		nav:true,
		margin: 0,
