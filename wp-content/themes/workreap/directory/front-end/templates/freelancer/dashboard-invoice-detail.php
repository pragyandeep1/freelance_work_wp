...
if( function_exists('wmc_revert_price')) {
	$get_total	= workreap_price_format(wmc_revert_price($get_total,$order->get_currency()));
}else{
	$get_total	= workreap_price_format($get_total);
}
...
if(!empty($order->get_items())){
	foreach ( $order->get_items() as $item_id => $item ) {
		...
		$html	.= '<tr>
						...
						<td data-label="'. esc_html__('Description', 'workreap').'"><span>'.$project_title.'</span>'.$addons.'</td>
		...
		<div class="wt-printable">
			...
			<div class="wt-tasksinfos">
				...
				<div class="wt-tasksdates">
					<span><em><?php esc_html_e('Issue date:', 'workreap'); ?></em>&nbsp;<?php echo date_i18n($date_format.', '.$time_format,strtotime($data_created));?></span>
				</div>
			</div>
